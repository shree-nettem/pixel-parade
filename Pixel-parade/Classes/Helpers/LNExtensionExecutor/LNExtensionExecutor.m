...
@property (strong, nonatomic) void (^presentedExtensionCompletion)(void);
...
- (void)presentViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion
{
    ...
    void (^newCompletion)(void) = ^(void) {
        self.presentedExtensionCompletion();
        completion();
    };

	[super presentViewController:viewControllerToPresent animated:flag completion: newCompletion];
}
...
{
	[self _executeWithInputItems:inputItems retriesCount:3 completionHandler:^ (BOOL completed, NSArray * __nullable returnedExtensionItems, NSError * __nullable activityError) {
...
- (void)_executeWithInputItems:(nonnull NSArray *)inputItems retriesCount:(NSUInteger)retriesCount completionHandler:(void (^ __nullable)(BOOL completed, NSArray* __nullable returnedItems, NSError* __nullable activityError))handler presentCompletionHandler: (void (^ __nullable)(void))presentCompletionHandler
{
    ...
    vc.presentedExtensionCompletion = ^(void) {
	...
	{
        self->_extensionPresentationWindow.hidden = YES;
        self->_extensionPresentationWindow = nil;
