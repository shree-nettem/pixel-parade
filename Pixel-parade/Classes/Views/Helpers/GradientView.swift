...
@IBDesignable open class GradientView: UIView {
    ...
    open var mode: Mode = .linear { //@IBInspectable
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    open var direction: Direction = .vertical { //@IBInspectable 
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    private func gradientColors() -> [UIColor]? {
        guard tintAdjustmentMode == .dimmed else { return colors }
        guard dimmedColors == nil else { return dimmedColors }
        guard !automaticallyDims else { return colors }
